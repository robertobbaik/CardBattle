...
```text
선택한 적 카드에게 자신의 현재 HP 50% 피해를 준다.
선택한 대상을 제외한 나머지 공개된 적 카드들에게 자신의 현재 HP 30% 피해를 준다.
반격 피해를 받지 않는다.
```
...
- 선택한 대상에게 `CurrentHP * 0.5` 피해를 적용한다.
- 선택한 대상을 제외한 공개된 적 카드들에게 `CurrentHP * 0.3` 피해를 적용한다.
- 피해 계산 결과가 0보다 크고 1보다 작으면 최소 1 피해를 적용한다.
- 대기 카드와 공개되지 않은 카드는 피해 대상이 아니다.
...
Enemy AI는 단순히 가장 왼쪽 카드를 고르거나 완전 랜덤으로 고르지 않는다.
`EnemyController`에서 공격 가능한 모든 적 카드와 공격 가능한 모든 플레이어 카드를 조합해 행동 후보를 만들고, 각 후보에 점수를 매긴 뒤 가장 높은 점수의 행동을 실행한다.

동일한 최고 점수 후보가 여러 개면 그 후보들 중 하나를 랜덤 선택한다.
이렇게 해서 AI가 너무 예측 가능하지 않으면서도 사람이 플레이하는 것처럼 유리한 공격을 고르게 한다.

```text
1. 살아있고 공개되어 있으며 공격 가능한 Enemy 카드 목록을 만든다.
2. 살아있고 공개된 Player 카드 목록을 만든다.
3. 모든 Enemy attacker / Player target 조합을 평가한다.
4. 최고 점수 조합을 선택한다.
5. 최고 점수 조합이 여러 개면 그 안에서 랜덤 선택한다.
6. 선택된 attacker가 선택된 target을 공격한다.
```

### 12.2 Enemy AI 점수 규칙

행동 후보 점수는 아래 기준을 함께 고려한다.

- 킬 가능한 대상이면 크게 가산한다.
- HP가 낮은 대상일수록 우선 공격한다.
- 힐러 / 지휘관 / 주술사처럼 유지력, 버프, 디버프에 영향을 주는 카드를 우선 제거한다.
- 암살자 / 무쌍 / 폭탄처럼 전투 위협도가 높은 카드도 우선 제거 대상으로 본다.
- 원거리 공격자는 반사 피해를 받지 않으므로 공격자 우선순위를 높게 본다.
- 반사 피해가 큰 공격은 감점한다.
- 버서커 공격의 자해 피해도 감점한다.
- 반사 피해와 자해 피해로 공격자가 죽을 가능성이 높으면 추가 감점한다.
- 무쌍 공격은 대상 주변에 추가 피해를 줄 수 있는 카드가 있으면 가산한다.
- 폭탄 공격은 광역 피해를 받을 카드가 많을수록 가산한다.

### 12.3 공격자 우선순위

공격자 자체의 안정성과 효율도 점수에 반영한다.

- 원거리는 반사 피해를 받지 않으므로 가장 높은 공격자 우선순위를 가진다.
- 폭탄은 반사 피해가 없고 광역 피해가 있으므로 추가 우선순위를 가진다.
- 무쌍은 인접 피해를 줄 수 있으므로 낮은 추가 우선순위를 가진다.

### 12.4 반사 피해 예측 기준

Enemy AI는 실제 공격 전에 대략적인 반사 피해를 예측한다.

- 원거리와 폭탄 공격은 반사 피해를 0으로 본다.
- 일반 / 힐러 / 지휘관 / 주술사 / 버서커는 대상의 현재 HP만큼 반사 피해를 받을 것으로 본다.
- 수호자와 무쌍은 대상 현재 HP의 120%를 반사 피해로 본다.
- 암살자는 대상 현재 HP의 150%를 반사 피해로 본다.
- 암살자가 공격으로 대상을 처치할 수 있으면 반사 피해를 0으로 본다.

### 12.5 공격 피해 예측 기준

Enemy AI는 카드별 실제 공격 로직과 맞춰 공격 피해를 예측한다.

- 일반 / 원거리 / 무쌍 / 힐러 / 암살자 / 주술사 / 지휘관은 현재 HP만큼 피해를 준다고 본다.
- 수호자는 현재 HP의 80%를 피해로 본다.
- 버서커는 현재 HP의 120%를 피해로 본다.
- 폭탄은 주 대상에게 현재 HP의 50%, 나머지 공개 적 카드에게 현재 HP의 30%를 피해로 본다.
- 모든 예측 피해는 최소 1로 계산한다.
