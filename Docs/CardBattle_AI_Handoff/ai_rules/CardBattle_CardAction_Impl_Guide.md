...
선택한 카드는 `공격` 또는 `스킬` 중 하나의 행동만 수행할 수 있다.
행동이 끝나면 턴이 종료된다.
...
```text
힐러의 턴 시작 회복은 자동 효과다.
힐러가 자동 회복을 발동했더라도, 플레이어는 그 턴에 카드 1장의 공격 또는 스킬을 사용할 수 있다.
```
...
| 스킬 | 일부 카드가 공격 대신 사용할 수 있는 특수 행동 |
...
public abstract class BaseCard
{
    ...
    public virtual void UseSkill(BaseCard target, BattleContext context)
    {
    }
...
버튼 위치는 항상 유지한다.
이유는 카드마다 UI 레이아웃이 바뀌면 사용자가 혼란스러울 수 있기 때문이다.
...
- 공격형 스킬 대상: 적 카드
- 버프형 스킬 대상: 아군 카드
- 자기 대상 스킬: 대상 선택 없이 즉시 확인 가능

### 10.3 행동 확정 후

공격 또는 스킬이 확정되면 다음을 수행한다.
...
```text
1. 살아있는 카드 중 랜덤 선택
2. 해당 카드가 스킬을 사용할 수 있으면 50% 확률로 스킬 선택
3. 적 대상 또는 아군 대상을 랜덤 선택
4. 행동 실행
```
...
- 수호자는 아군이 많을 때 방어 태세 우선 사용
- 지휘관은 무쌍 / 광전사에게 버프 우선 사용
...
- 일반 카드가 기본 공격을 수행한다.
- 원거리 카드가 반격 없이 공격한다.
- 무쌍 카드가 스킬로 대상 + 인접 피해를 준다.
- 힐러가 턴 시작 시 아군을 회복한다.
- 수호자가 방어 태세를 사용할 수 있다.
- 암살자가 처형 스킬을 사용할 수 있다.
- 광전사가 광폭 일격을 사용할 수 있다.
- 주술사가 약화 저주를 사용할 수 있다.
- 지휘관이 아군에게 격려를 부여할 수 있다.
- 폭탄병이 폭탄 투척과 사망 폭발을 처리한다.

### 14.2 행동 제한

- 한 턴에 선택한 카드는 공격 또는 스킬 중 하나만 사용한다.
- 행동 후 턴이 종료된다.
- 패시브, 반격, 사망 효과는 행동 횟수를 소모하지 않는다.

### 14.3 UI

- 카드 선택 시 공격 / 스킬 / 취소 버튼이 표시된다.
- 스킬이 없는 카드의 스킬 버튼은 비활성화된다.
- 공격 대상은 적 카드만 선택된다.
- 버프 스킬 대상은 아군 카드만 선택된다.
- 행동 후 UI 상태가 초기화된다.
